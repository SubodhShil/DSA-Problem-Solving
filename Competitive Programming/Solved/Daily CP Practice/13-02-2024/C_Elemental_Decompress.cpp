...
void solve()
{
    ...
    map<int, int> arrMap, permutationMap;
    map<int, vector<int>> idxOccur;
    vector<int> arr(n, 0), p1(n, 0), p2(n, 0);
    bool ok = 1;

    for(int i = 0; i < n; ++i) 
    {
        cin >> arr[i];
        arrMap[arr[i]]++, permutationMap[i + 1] = 1, p1[i] = arr[i];
        if(arrMap[arr[i]] > 2) ok = 0;
    }
